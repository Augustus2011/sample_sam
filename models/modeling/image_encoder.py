...
class ImageEncoderViT(nn.Module):
    def __init__(self, args, image_size: int = 1024, patch_size: int = 16, in_chans: int = 3, embed_dim: int = 768, depth: int = 12, num_heads: int = 12, mlp_ratio: float = 4.0,
                 out_chans: int = 256, qkv_bias: bool = True, norm_layer: Type[nn.Module] = nn.LayerNorm, act_layer: Type[nn.Module] = nn.GELU,
                 use_abs_pos: bool = True, use_rel_pos: bool = False, rel_pos_zero_init: bool = True, window_size: int = 0,
        ...
        if use_abs_pos:
            self.pos_embed=nn.Parameter(torch.zeros(1,image_size//patch_size,image_size//patch_size,
                                                    embed_dim,dtype=torch.float,device=self.dev[0]))
        ...
        self.neck=nn.Sequential(nn.Conv2d(embed_dim,out_chans,kernel_size=1,bias=False),LayerNorm2d(out_chans),
    ...
    def forward(self,x:torch.Tensor)->torch.Tensor:
        ...
        for i,blk in enumerate(self.blocks):
            if self.args.if_split_encoder_gpus:
                if i<int(self.depth*self.args.gpu_frations[0]):
                    x=x.to(self.dev[0])
                else:
                    x=x.to(self.dev[1])
            x=blk(x)
...
class Block(nn.Module):
    ...
    def __init__(self,args,dim:int,num_heads:int,mlp_ratio:float=4.0,scale:float=0.5,qkv_bias:bool=True,
                 norm_layer:Type[nn.Module]=nn.LayerNorm,act_layer:Type[nn.Module]=nn.GELU,
                 depth=1,use_rel_pos:bool=False,rel_pos_zero_init:bool=True,window_size:int=0,
...
class Attention(nn.Module):
    def __init__(self,dim:int,num_heads:int=8,qkv_bias:bool=True,use_rel_pos:bool=False,res_pos_zero_init:bool=True,
                 input_size:Optional[Tuple[int,int]]=None,)->None:
        super().__init__()
